  ...
  return (
    <div className="panel panel-default">
      <div className="panel-heading">
        <Link to={`/${status.user.username}`} className="status-user-link">{status.user.username}</Link>
      </div>
      ...
      <ul>
        ...
        <div className="panel-heading">
          Posted: {moment(status.createdAt).format('hh:mm:ss | DD/MM/YYYY')}
          {del ? <button className="btn btn-danger btn-sm fr" onClick={click}>DELETE</button> : null }
        </div>
    </div>
  );
};
